...
class PlayState extends MusicBeatState
{
	...
	private function handleInput(evt:KeyboardEvent):Void
	{
		...
		var canMiss:Bool = !FlxG.save.data.ghost && songStarted;
	...
	override public function update(elapsed:Float)
	{
		...
		if (generatedMusic && !(inCutscene || inCinematic))
		{
			...
			var leSpeed = PlayStateChangeables.scrollSpeed == 1 ? SONG.speed : PlayStateChangeables.scrollSpeed;
	...
	private function popUpScore(daNote:Note):Void
	{
		...
		if (daRating == 'sick' && doNoteSplash && daNote.canNoteSplash || daRating == 'marv' && doNoteSplash && daNote.canNoteSplash)
		{
		...
		if (FlxG.save.data.popup)
		{
			createTween(rating, {alpha: 0}, 0.2, {startDelay: (Conductor.crochet * Math.pow(songMultiplier, 2)) * 0.001});
	...
	override function beatHit()
	{
		...
		if (FlxG.save.data.camzoom && songMultiplier == 1)
		{
			...
			{
				FlxG.camera.zoom += 0.015 * zoomMultiplier / songMultiplier;
				camHUD.zoom += 0.03 * zoomMultiplier / songMultiplier;
			}
