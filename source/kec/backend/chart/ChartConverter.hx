...
class ChartConverter
{
	...
	{
		Debug.logTrace("You've Done Something Very Wrong To Be Seeing This.");
		final data:Legacy = cast song.song;
		data.eventObjects = checkEvents(data);
		if (data.audioFile == null)
			data.audioFile = data.song.toLowerCase();
		if (data.songId == null)
			data.songId = data.song.toLowerCase();
	...
	public static function convertPsychV1(song:Dynamic):Modern
	{
		...
		final songName = data.song.replace(" ", "-");
		final newSong:Modern = {
			songName: data.song,
			songId: songName.toLowerCase(),
			audioFile: songName.toLowerCase(),
			notes: newNotes,
			player1: data.player1,
			player2: data.player2,
			gfVersion: data.gfVersion,
